...
class Model_ticket extends CI_Model{
  ...
  function get_ticket($nim){
    ...
    on `ticket`.`nip_dosen` = `dosen`.`nip_dosen`  WHERE `mahasiswa`.`nim_mahasiswa`=".$nim." ORDER BY ticket_time DESC";
    return $this->db->query($sql)->result();
  }

  function get_ticket_dosen($nip){
    $sql = "SELECT * FROM `ticket` JOIN `mahasiswa` on `ticket`.`nim_mahasiswa`
    = `mahasiswa`.`nim_mahasiswa` JOIN `dosen`
    on `ticket`.`nip_dosen` = `dosen`.`nip_dosen` WHERE `dosen`.`nip_dosen`=".$nip." ORDER BY ticket_time DESC";
    return $this->db->query($sql)->result();
  }
